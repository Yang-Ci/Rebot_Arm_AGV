...
- `rebotarm_mujoco_rs/` 从 `/home/robot/reBot_Arm_Mujoco-RS` 复制后修改，默认加载 `rs_agv_scene.xml`，支持 `cmd_vel` 驱动 AGV、同步底盘位姿、`/odom`、`/scan` 和导航 TF。

## 本地仿真

```bash
python3 tools/build_rs_agv_model.py
colcon build --packages-select rebotarm_msgs rebotarm_mujoco_rs
...
组合场景带一个前置 2D lidar 占位件和 2 m x 2 m 外围墙。仿真桥发布
`/odom`、`/scan` 和 `odom -> base_footprint -> base_link -> laser` TF，接口
约定参考 [linorobot2](https://github.com/linorobot/linorobot2)，后续可以直接接
SLAM Toolbox 或 Nav2。linorobot2 仅作为只读参考，源码没有复制进本仓库。

默认速度入口采用 linorobot2/Nav2 约定的 `/cmd_vel`。初始位姿正前方是机械臂
操作台，因此上面的独立行驶测试先向后退；需要保留旧命名空间话题时，可在启动
命令中增加 `cmd_vel_topic:=/rebotarm_rs/cmd_vel`。
